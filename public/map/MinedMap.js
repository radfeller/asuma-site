...
window.createMap = function () {
	(async function () {
		...
		const southWest = L.latLng(-maxZ, minX);
		const northEast = L.latLng(-minZ, maxX);
...
//				map.removeLayer(lightLayer);
			...
			updateHash();
			// Принудительно выставляем центр и масштаб — игнорируя hash
		const minX = 17, maxX = 577;
		const minZ = 25, maxZ = 701;
		const southWest = L.latLng(-maxZ, minX);
		const northEast = L.latLng(-minZ, maxX);
		const bounds = L.latLngBounds(southWest, northEast);
		map.setMaxBounds(bounds);
